...
import {
  ...
  Target, Calendar, ChevronDown, Percent, BadgeDollarSign,
} from 'lucide-react'
...
const BOOKING_STATUS_COLORS: Record<string, string> = {
  ...
  CHECKED_IN: 'text-purple-700 bg-purple-50 border-purple-200',
  COMPLETED:  'text-emerald-700 bg-emerald-50 border-emerald-200',
  CANCELLED:  'text-red-600 bg-red-50 border-red-200',
  NO_SHOW:    'text-gray-500 bg-gray-100 border-gray-200',
}

const BOOKING_STATUS_LABELS: Record<string, string> = {
  PENDING: 'Chờ xác nhận', CONFIRMED: 'Đã xác nhận', CHECKED_IN: 'Nhận phòng',
  COMPLETED: 'Hoàn thành', CANCELLED: 'Đã huỷ', NO_SHOW: 'No-show',
}
...
function RevenueTooltip({ active, payload, label }: CustomTooltipProps) {
  ...
  const moneyKeys = ['Doanh thu gộp', 'Hoa hồng', 'Net KS', 'Doanh thu', 'revenue', 'gross', 'net', 'commission', 'value']
  return (
    <div className="bg-white border border-gray-200 rounded-xl shadow-xl px-4 py-3 min-w-[170px]">
      <p className="font-bold text-gray-600 mb-2 text-xs">{label}</p>
      {payload.map((p, i) => {
        const name = p.name ?? ''
        const val  = Number(p.value ?? 0)
        const isMoney = moneyKeys.some(k => name.includes(k) || k === name)
        return (
          <div key={i} className="flex items-center justify-between gap-4 mb-1">
            ...
            <span className="font-semibold text-gray-900 text-xs">
              {isMoney ? val.toLocaleString('vi-VN') + '₫' : val}
            </span>
          </div>
        )
      })}
...
function StatCard({ icon: Icon, label, value, sub, color }: StatCardProps) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm hover:shadow-md transition-all group">
      ...
      <p className="text-xs font-medium text-gray-400 mt-2">{sub}</p>
    </div>
  )
}
  ...
  return (
    <div className="p-3 rounded-xl border border-gray-100 bg-gray-50/40 hover:bg-white hover:shadow-sm transition-all">
      ...
      <div className="flex items-center justify-between mt-1">
        <p className="text-[11px] text-emerald-600 font-bold">{Number(b.totalAmount).toLocaleString('vi-VN')}₫</p>
        <p className="text-[10px] text-gray-400">{new Date(b.createdAt).toLocaleDateString('vi-VN')}</p>
      </div>
    </div>
  )
}
...
function SectionHeader({ color, icon: Icon, title, onExport, isExporting }: {
  color: 'blue' | 'emerald'
  icon: React.ComponentType<{ size?: number; className?: string }>
  title: string; onExport: () => void; isExporting: boolean
}) {
  return (
    <div className="flex items-center gap-2">
      ...
      <button onClick={onExport} disabled={isExporting}
        className={cn('ml-auto text-xs font-bold flex items-center gap-1.5 px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50',
          color === 'blue' ? 'text-emerald-600 bg-emerald-50 hover:bg-emerald-100' : 'text-blue-600 bg-blue-50 hover:bg-blue-100')}>
...
function DateFilter({ mode, month, year, fromDate, toDate, onModeChange, onMonthChange, onYearChange, onFromChange, onToChange }: {
  ...
  onYearChange: (v: number) => void; onFromChange: (v: string) => void; onToChange: (v: string) => void
}) {
  return (
    <div className="flex flex-col items-end gap-2">
      ...
      {mode === 'month' ? (
        <div className="flex items-center gap-2">
          {[
            { items: MONTHS, value: month, onChange: onMonthChange, fmt: (v: number) => `Tháng ${v}` },
            { items: YEARS,  value: year,  onChange: onYearChange,  fmt: (v: number) => String(v)     },
          ].map((sel, i) => (
            <div key={i} className="relative">
              <select value={sel.value} onChange={e => sel.onChange(Number(e.target.value))}
                ...
                {sel.items.map(v => <option key={v} value={v}>{sel.fmt(v)}</option>)}
              </select>
        ...
        <div className="flex items-center gap-2">
          {[{ value: fromDate, onChange: onFromChange }, { value: toDate, onChange: onToChange }].map((inp, i) => (
            <div key={i} className="relative">
...
export default function OwnerDashboardPage() {
  ...
  const [fromDate, setFromDate] = useState<string>(() => { const d = new Date(); d.setDate(1); return d.toISOString().split('T')[0] })
  ...
  const handleExport = async (isGlobal: boolean) => {
    ...
    } catch { toast.error('Có lỗi xảy ra khi xuất file') }
  ...
  const hotelPieData = useMemo<PieDataPoint[]>(() =>
    (globalData?.topHotels ?? [])
      ...
      .map((h: HotelStatisticSummaryResponse) => ({ name: h.hotelName, value: Number(h.grossRevenue) })),
    [globalData]
  )
  ...
  const barData = useMemo<BarDataPoint[]>(() =>
    (globalData?.topHotels ?? []).map((h: HotelStatisticSummaryResponse) => ({
      ...
      gross:      Number(h.grossRevenue     ?? 0),
      net:        Number(h.netRevenue       ?? 0),
      commission: Number(h.totalCommission  ?? 0),
    ...
    <div className="space-y-10 pb-20">
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
          ...
          <StatCard icon={BadgeDollarSign} color="green"  label="Net KS nhận"     value={fmtFull(Number(gs?.netRevenue      ?? 0))} sub="Sau trừ hoa hồng" />
          <StatCard icon={CalendarCheck}   color="purple" label="Hoàn thành"       value={String(gs?.completedBookings ?? 0)}         sub="Booking thành công" />
          <StatCard icon={XCircle}         color="red"    label="Đã huỷ"           value={String(gs?.totalCancelled    ?? 0)}         sub="Tất cả KS" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 p-6">
            ...
            <p className="text-xs text-gray-400 mb-5">Doanh thu gộp / Net KS nhận / Lượt đặt thành công</p>
            {barData.length === 0
              ...
              : (
                <ResponsiveContainer width="100%" height={280}>
                  <BarChart data={barData} barGap={2}>
                    ...
                    <YAxis yAxisId="cnt" orientation="right" tick={{ fontSize: 10 }} axisLine={false} tickLine={false} width={28} />
                    <Tooltip content={<RevenueTooltip />} />
                    <Legend wrapperStyle={{ fontSize: 11, fontWeight: 600 }} />
                    <Bar yAxisId="rev" dataKey="gross"    name="Doanh thu gộp" radius={[4, 4, 0, 0]} maxBarSize={32}>
                      {barData.map((_: BarDataPoint, i: number) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}
                    </Bar>
                    <Bar yAxisId="rev" dataKey="net"      name="Net KS nhận"  radius={[4, 4, 0, 0]} fill="#10b981" maxBarSize={32} />
                    <Bar yAxisId="cnt" dataKey="bookings" name="Lượt đặt"     radius={[4, 4, 0, 0]} fill="#CBD5E1" maxBarSize={16} />
                  </BarChart>
                </ResponsiveContainer>
              )}
          </div>
      ...
      <div className="space-y-5 pt-6 border-t border-dashed border-gray-200">
        ...
        {!isLocalLoading && ls && (
          <>
            <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
              <StatCard icon={TrendingUp}      color="blue"   label="Doanh thu gộp"  value={fmtFull(Number(ls.grossRevenue    ?? 0))} sub={activeHotel?.hotelName ?? ''} />
              <StatCard icon={Percent}         color="amber"  label="Hoa hồng"        value={fmtFull(Number(ls.totalCommission ?? 0))} sub="Thu của hệ thống" />
              <StatCard icon={BadgeDollarSign} color="green"  label="Net KS nhận"    value={fmtFull(Number(ls.netRevenue      ?? 0))} sub="Sau trừ hoa hồng" />
              <StatCard icon={CalendarCheck}   color="purple" label="Hoàn thành"      value={String(ls.completedBookings ?? 0)}          sub="Booking" />
              <StatCard icon={XCircle}         color="red"    label="Đã huỷ"          value={String(ls.totalCancelled    ?? 0)}          sub="Phòng" />
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
              <div className="lg:col-span-8 bg-white rounded-2xl border border-gray-100 p-6">
                ...
                <p className="text-xs text-gray-400 mb-5">Gross / Hoa hồng / Net theo ngày trong kỳ</p>
                  ...
                  : (
                    <ResponsiveContainer width="100%" height={280}>
                      <AreaChart data={areaChartData}>
                        ...
                        <Area type="monotone" dataKey="gross"      name="Doanh thu gộp" stroke="#3b82f6" strokeWidth={2}   fill="url(#gradGross)" dot={false} />
                        <Area type="monotone" dataKey="commission" name="Hoa hồng"       stroke="#f59e0b" strokeWidth={1.5} fill="none"            dot={false} strokeDasharray="5 3" />
                        <Area type="monotone" dataKey="net"        name="Net KS nhận"   stroke="#10b981" strokeWidth={2}   fill="url(#gradNet)"   dot={false} />
                      </AreaChart>
                    </ResponsiveContainer>
                  )}
              </div>
