...
interface HotelChatWidgetProps {
    hotelId: number
    hotelName: string
    hotelOwnerEmail: string  // ✅ Bắt buộc — cần để gửi đến đúng receiver
}
...
export default function HotelChatWidget({ hotelId, hotelName, hotelOwnerEmail }: HotelChatWidgetProps) {
    ...
    // ── Dùng ref để tránh stale closure trong WebSocket handler ──────────
    ...
    // ── Kết nối WebSocket 1 lần khi user login ────────────────────────────
    useEffect(() => {
        ...
        const client = new Client({
            ...
            onConnect: () => {
                ...
                client.subscribe('/user/queue/messages', (frame) => {
                    ...
                    // Sync conversationId từ message đầu tiên của conversation mới
                    if (msg.conversationId && conversationIdRef.current === null) {
                        conversationIdRef.current = msg.conversationId
                    }

                    // Bỏ qua tin không thuộc hotel đang chat
                    if (msg.conversationId !== conversationIdRef.current) return

                    setMessages(prev => {
                        // ✅ Replace optimistic: tìm message tạm (id < 0) cùng content + senderEmail
        ...
        try {
            stompClientRef.current!.publish({
                ...
                body: JSON.stringify({
                    ...
                    hotelId,
                    content,
                    // ✅ Fix: truyền đúng receiverEmail để backend gửi cho hotel owner
    ...
    return (
        <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
                                ...
                                return (
                                    <div key={msg.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                                        ...
                                        <div className={`max-w-[75%] flex flex-col gap-0.5 ${isMine ? 'items-end' : 'items-start'}`}>
                                            <div className={`px-3 py-2 rounded-2xl text-sm leading-relaxed transition-opacity ${isOptimistic ? 'opacity-60' : 'opacity-100'} ${isMine
                                                ? 'bg-blue-600 text-white rounded-br-sm'
                                                : 'bg-white text-gray-800 border border-gray-100 rounded-bl-sm shadow-sm'
                                            }`}>
                                                {msg.content}
                                            </div>
                                            <span className="text-[10px] text-gray-400 px-1">{fmt(msg.timestamp)}</span>
                                        </div>
                                    </div>
                                )
                            })
            ...
            <button
                ...
                className={`w-14 h-14 rounded-full flex items-center justify-center transition-all active:scale-95 relative ${isOpen ? 'bg-gray-700 hover:bg-gray-800' : 'bg-blue-600 hover:bg-blue-700'}`}
